...
all_tps_data <- readland.tps("data/bee_wings3.TPS",
...
xval <- xvalDapc(lm_values, lm_data$Species, n.pca.max = 300, training.set = 0.9, result = "groupMean", center = TRUE, scale = FALSE, n.pca = NULL, n.rep = 30, xval.plot = TRUE)
...
scatter(dapc_1,1,1,bg="white", #1,1, = 1st discriminant function
        ...
        legend=TRUE, size.leg=4, posi.leg="topright", col = species_col) # plot w/ just one axis meaning just 1 DA
...
sub_col <- c("red4", "darkblue", "darkcyan", "forestgreen", "gold3", "magenta4")
...
#genus_data <- population[!lm_data$Genus %in% excluded_genus, ]
#genus_groups <- lm_data$Genus[!lm_data$Genus %in% excluded_genus]

included_genus <- c("Apis", "Xylocopa","Bombus", "Ceratina", "Eucera", "Diadasia") #Subset "X" species from data
...
genus_col <- c( "darkblue", "forestgreen", "red4","darkcyan",  "magenta4") #save colors for population
...
species_col <- c("darkblue", "darkcyan", "forestgreen", "red4", "magenta4", "chartreuse4", "gold3") #save colors for population

scatter(dapc_spe, scree.da = FALSE, posi.da = "bottomleft",
        bg="white", pch=20, 
        cstar=0, solid=.4, cex=3, scree.pca=FALSE, posi.pca="bottomleft",
        legend=TRUE, posi.leg = "topright",
        col = species_col)

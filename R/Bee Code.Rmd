...
manova_species <- subset_lm #subset
manova_species <- manova_species[,c(2:23)] #drop catalog number and other non LM variables
...
fit_species <- lm.rrpp(manova_species_data ~ Genus, SS.type = "I", #Species, Genus, Subfamily, Family
...
summary(fitm_species, test = "Wilks") #summarize with a manova table
summary(fitm_species, test = "Hotelling-Lawley") #summarize with a manova table
...
#lm_values <- subset_lm[,c(2:19)] # Subset
...
dapc_2 <- dapc(population, 
                grp = lm_data$Species, #Species, Genus, Subfamily, Family, 
...
scatter(dapc_2, 1,2, #1,2, = 1st and 2nd discriminant functions
        ...
        col = dapc_col, clabel = 0.5) #plot the two populations
...
scatter(dapc_sub, scree.da = FALSE, posi.da = "bottomleft",
        bg="white", pch=20, 
        cstar=0, solid=.45, cex=2.5, clab = 0.5, scree.pca=FALSE, posi.pca="topbottom",
        legend=TRUE, posi.leg = "bottomright",
        col = sub_col)

summary.dapc(dapc_sub)
print.dapc(dapc_sub)
```
...
dapc_gen <- dapc(genus_data, 
                grp = genus_groups, 
                n.pca = 8, #Axes retained in Principal Component Analysis (PCA)
                n.da = 10) #Axes retained in Discriminant Analysis
...
summary.dapc(dapc_spe)
print.dapc(dapc_spe)
```

#################################################################################################

#DELETE CHUNKS AFTER THIS
...
lm_data$Family <- factor(lm_data$Family,
                       levels = c("Apidae","Andrenidae","Megachilidae", "Halictidae", "Melittidae"))
...
landmark_data <- matrix(c(1.2, 3.4, 2.3, 4.5, 3.1, 2.6,
                          2.0, 4.0, 3.5, 5.2, 4.0, 3.4), 
                        nrow = 2, byrow = TRUE)

# Create a species or specimen label for each row (each specimen)
species_labels <- c("Specimen1", "Specimen2")

# Convert data to a dataframe with labels
landmark_df <- data.frame(species = species_labels, landmark_data)

############################################

# Prepare data for NEXUS file
nexus_data <- data.frame(species = species_labels, landmark_data)

# Write the data to a NEXUS file
nexus_file <- "output/morphometric_data.nex"

write.nexus.data(nexus_data, file = nexus_file)

############################################

# Write the data to a NEXUS file
nexus_file <- "output/bee_matrix.nex"

write.nexus.data(LM_matrix, file = nexus_file)

```

#CV Size-Corrected test
```{r}

CV_Allo <- read_csv("data/CV_Allometry.csv")

geom_point()

ggplot(CV_Allo, aes(x=CV1, y=CV2)) + geom_point()

ggplot(CV_Allo, aes(x=CV1, y=CV2, color=Genus)) + geom_point() + stat_ellipse()
